...
class Shape3D:
    ...
    def draw(self, screen, center, color, scale=100):
        ...
        for edge in self.edges:
            ...
            surf = pygame.Surface((abs(end_pos[0] - start_pos[0]) + 10, 
                                abs(end_pos[1] - start_pos[1]) + 10), pygame.SRCALPHA)
            
            # Draw line on surface
            start_on_surf = (5, 5) if start_pos[0] < end_pos[0] else (surf.get_width()-5, 5)
            end_on_surf = (surf.get_width()-5, surf.get_height()-5) if start_pos[0] < end_pos[0] else (5, surf.get_height()-5)
            
            try:
                pygame.draw.line(screen, edge_color[:3], start_pos, end_pos, 2)
            except:
    ...
    def draw_holographic_text(self, text, x, y, font, color, glow=True):
        if glow:
            ...
            for i in range(8, 0, -1):
                ...
                self.screen.blit(glow_surf, (x + i//2, y + i//2))
